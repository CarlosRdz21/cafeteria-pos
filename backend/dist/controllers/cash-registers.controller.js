...
class CashRegistersController {
    ...
    static async closeCurrent(req, res) {
        try {
    ...
    static async recordSaleCurrent(req, res) {
        try {
    ...
    static async recordExpenseCurrent(req, res) {
        try {
            const amount = toNumber(req.body?.amount, -1);
            if (amount < 0) {
                return res.status(400).json({ error: 'amount is required' });
    ...
    static async applySaleToOpenRegister(paymentMethod, amount) {
        const open = await db.cashRegister.findFirst({
            where: { status: 'open' },
            orderBy: { openedAt: 'desc' }
        });
